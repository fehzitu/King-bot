    ...
    async execute(message) {
        ...
        if (content.length == 2 && firstMentionedUser) {
            ...
            if (message.author.id == firstMentionedUser.id) {
                ...
                successEmbed.setDescription(`🤔 **<@${message.author.id}> quer se morder**❓`);
                successEmbed.setImage(img.url);
            } else {
                // get the img link
                img = await getImg('bite');

                // set the title of embed with mentioned username
                successEmbed.setDescription(`😡 **<@${message.author.id}> mordeu <@${firstMentionedUser.id}>**❗`);
                successEmbed.setImage(img.url);
            };
